#' This function modifies a dataset of simulated islands by removing random species from rich lineages.
...
remove_random_spec_from_rich_lineages <- function(data_sims_trees, rich_lineage_treshold, percentage_spec_to_remove, data_sims_G) {

  data_sims_G3 <- list()  # Initialize an empty list to store the modified datasets
  ...
  while (species_already_removed < spec_to_remove) {
    ...
    for (j in 2:length(one_island1)) {
      # Check if the current lineage has more branching times than the threshold and if species are still left to remove
      if (length(one_island[[j]]$branching_times) > rich_lineage_treshold && species_already_removed < spec_to_remove) {

        # Handle lineages with more than 2 branching times
        ...
        # Handle lineages with exactly 2 branching times and more than 2 branching times in the original data
        ...
        # Handle lineages with exactly 2 branching times and the original data also has 2 branching times
        else if (length(one_island1[[j]]$branching_times) == 2 && length(one_island[[j]]$branching_times) > 2 && species_already_removed < spec_to_remove && one_island1[[j]]$branching_times[2] == col_max) {
          one_island1[[j]]$branching_times <- c(island_age, col_max)
        }
      }
    }
    # Assign the modified dataset to the output list
